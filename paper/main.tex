...
We call a retained target a \emph{source-loss target} when its matched tool value is no longer
available under this final attention structure. If several matched values connect the same tool
observation and assistant message, they are treated as one target unit, and the target is counted
as source-loss if any of those values loses its recorded source.

We report three quantities. First, the source-loss rate is the fraction of retained matched targets
with source loss. Second, we report the fraction of original conversations containing at least one
source-loss target. Third, we report the affected target-value tokens as a fraction of the original
assistant loss tokens. The first measures the failure among the relations captured by the matcher,
the second shows how widely it occurs across conversations, and the third gives its contribution
to the full SFT loss.
...
\begin{figure}[t]
  ...
  \caption{
  Separation of matched assistant targets from their recorded tool observations across token
  budgets. Panel A shows the fraction of retained matched targets whose tool value is no longer
  available after BFD-split or intended wrapped packing. Default BFD produces no such cases among
  retained matched targets in any corpus--budget combination. Panel B compares the 512-token
  separation rate with the fraction of original assistant supervision retained.
  }
  \label{fig:policy}
\end{figure}
...
\small
\caption{
Results at a 512-token budget. ``Separated'' counts retained matched targets whose recorded
tool value is no longer available to the supervised target. ``Affected assistant tokens'' is
the number of affected target-value tokens divided by the original assistant loss-token count.
}
\label{tab:primary}
\resizebox{\linewidth}{!}{
...
\toprule
Corpus & Policy &
Matched targets retained &
Separated &
Affected conversations &
Affected assistant tokens &
Assistant supervision retained \\
\midrule
ToolACE
& BFD
& 10/918
...
\begin{figure}[t]
  ...
  \caption{
  Matched-target retention versus retained input-token ratio for three ways of reconstructing
  training examples at 256, 512, 1,024, and 2,048 tokens. Grouping targets from the same
  conversation generally matches one-example-per-target reconstruction using fewer input tokens.
  At 512 tokens, grouping reduces duplicated source tokens from 1,244 to zero in ToolACE and
  from 1,848 to 311 in ReTool.
  }
  \label{fig:grouped}
\end{figure}
...
\begin{figure}[t]
  ...
  \caption{
  Fraction of matched tool-call targets that fit within the token budget together with the
  relevant user constraint, complete tool call, and specified schema content. Reducing the schema
  substantially increases short-budget coverage in ToolACE, whereas ReTool's compact schemas
  are better retained in full.
  }
  \label{fig:schema}
\end{figure}
...
The accompanying artifact contains pinned dataset and model revisions, content hashes, experiment
configurations, random seeds, raw policy outputs, reconstructed training examples, and model
predictions. Automated tests cover chat-template rendering, source and target span mapping, token
tracking, packed sequence boundaries, training-example reconstruction, schema validity, and
train/development/test separation. The analyzed corpora and Qwen2.5 model are publicly released
under permissive licenses. The study uses no private conversations or human-subject data.
...
Dataset and model revisions, licenses, and reproducibility details are provided in the appendix
and accompanying reproducibility artifact.

\begin{appendices}
\section{Reproducibility}

The accompanying artifact contains the code, configurations, and recorded outputs used for
...
and the 5,000 lowest-ranked conversations form the evaluation sample.
...
\resizebox{\textwidth}{!}{\input{../tables/table1_contemporary_512.tex}}
\end{table}
...
\label{tab:case512}
\resizebox{\textwidth}{!}{\input{../tables/tableS2_case_sensitive_512.tex}}
\end{table}
...
reconstructed as valid structures are not counted as covered.
...
\resizebox{0.85\textwidth}{!}{\input{../tables/table3_schema_repair.tex}}
\end{table}
...
\caption{
Controlled source-removal experiment across three optimization seeds. Exact generation is
100\% at 25\% and 50\% source removal and falls to zero when the tool value is removed from
all training examples. In the 0\% condition, one seed frequently emits the correct value using
\texttt{VALUE:} instead of the prespecified \texttt{VALUE=} format, reducing its strict exact
score.
}
\end{figure}
